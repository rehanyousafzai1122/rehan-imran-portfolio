...
export const SecretAdminPanel: React.FC = () => {
  ...
  // ----------------------------------------------------
  // Shared upload helper: pushes the file to Firebase Storage (so it's a real
  // shared URL every visitor can load) when Firebase is configured; otherwise
  // falls back to a local base64 data URL (single-browser only).
  // ----------------------------------------------------
  const uploadMedia = (
    file: File,
    folder: string,
    ...
    if (storage) {
      const path = `portfolio/${folder}/${Date.now()}-${file.name}`;
      const fileRef = storageRef(storage, path);
      uploadBytes(fileRef, file)
        .then((snapshot) => getDownloadURL(snapshot.ref))
        .then((url) => onUrl(url))
        .catch(() => onError?.('Upload failed — check your connection and try again.'));
      return;
    }

    // Fallback: base64 in local state/localStorage only (not shared across devices).
    const reader = new FileReader();
    reader.onload = (event) => {
      const dataUrl = event.target?.result as string;
      if (dataUrl) onUrl(dataUrl);
    };
  ...
  // File Upload Helper: uploads to Firebase Storage (or base64 fallback)
  ...
  const handleAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    uploadMedia(
      ...
      'avatar',
      (url) => {
        updatePersonalInfo({ avatarPortrait: url });
        setProfileForm((prev) => ({ ...prev, avatarPortrait: url }));
  ...
  const handleStudioUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    uploadMedia(
      ...
      'studio',
      (url) => {
        updatePersonalInfo({ studioScene: url });
        setProfileForm((prev) => ({ ...prev, studioScene: url }));
  ...
  const handleVideoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!storage) {
      const MAX_BYTES = 5 * 1024 * 1024; // ~5MB — safe ceiling for localStorage fallback
      if (file.size > MAX_BYTES) {
        showToast('Video too large for local storage — set up Firebase Storage or paste a hosted URL.');
        return;
      }
    }
    ...
    uploadMedia(
      ...
      'video',
      (url) => {
        updatePersonalInfo({ videoIntroUrl: url });
  ...
  const handleChangePasswordSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      showToast(result.error || 'Could not change passcode.');
    }
  };
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-[#161616]/90">
            ...
            <div className="flex items-center gap-2">
              {/* Cloud Sync Status */}
              <span
                className={`hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-mono uppercase tracking-wider ${
                  isCloudConnected
                ...
                title={
                  isCloudConnected
                    ? 'Changes save to the cloud database and show on every device.'
                    : 'Firebase not configured — changes only save on this browser.'
                }
              >
                <span className={`w-1.5 h-1.5 rounded-full ${isCloudConnected ? 'bg-emerald-400' : 'bg-amber-400'}`} />
                {isSyncing ? 'Syncing…' : isCloudConnected ? 'Synced to Cloud' : 'Local Only'}
              </span>
